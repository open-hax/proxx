...
test("classifyRateLimitKind: Ollama session limit → quota_exhausted", () => {
  ...
  assert.equal(classifyRateLimitKind(429, body, 5000), "quota_exhausted");
});

test("classifyRateLimitKind: Ollama weekly limit → quota_exhausted", () => {
  const body = { error: "Weekly usage limit reached" };
  assert.equal(classifyRateLimitKind(429, body, 60_000), "quota_exhausted");
});
...
test("classifyRateLimitKind: concurrent request limit message → concurrency_throttle", () => {
  ...
  assert.equal(classifyRateLimitKind(429, body, 5000), "concurrency_throttle");
});
...
test("classifyRateLimitKind: short retry-after without quota keywords → concurrency_throttle", () => {
  assert.equal(classifyRateLimitKind(429, undefined, 3000, 30_000), "concurrency_throttle");
});

test("classifyRateLimitKind: short retry-after with body → concurrency_throttle", () => {
  const body = { error: { message: "Slow down please" } };
  assert.equal(classifyRateLimitKind(429, body, 8000), "concurrency_throttle");
});
...
test("classifyRateLimitKind: quota-exhaustion message → quota_exhausted even with short retry", () => {
  ...
  assert.equal(classifyRateLimitKind(429, body, 3000), "quota_exhausted");
});
...
test("classifyRateLimitKind: credit exhaustion → quota_exhausted", () => {
  ...
  assert.equal(classifyRateLimitKind(429, body, 10_000), "quota_exhausted");
});

test("classifyRateLimitKind: long retry-after without indicators → quota_exhausted (safe default)", () => {
  assert.equal(classifyRateLimitKind(429, undefined, 120_000, 30_000), "quota_exhausted");
});

test("classifyRateLimitKind: no body, no retry-after → quota_exhausted (safe default)", () => {
  assert.equal(classifyRateLimitKind(429, undefined, undefined), "quota_exhausted");
});
...
test("classifyRateLimitKind: ZAI-style 'too many requests' → concurrency_throttle", () => {
  ...
  assert.equal(classifyRateLimitKind(429, body, 10_500), "concurrency_throttle");
});
...
test("classifyRateLimitKind with providerId falls back to generic heuristics when CLJS unavailable", () => {
  ...
  assert.equal(classifyRateLimitKind(503, body, 5000, 30000, "unknown-provider"), "concurrency_throttle");
});

test("classifyRateLimitKind with providerId still applies Ollama rules", () => {
  const body = { error: "Session usage limit reached" };
  assert.equal(classifyRateLimitKind(429, body, 5000, 30000, "ollama"), "quota_exhausted");
});
